...
from utils import BYTE_BITS, bin_join, bin_split
...
SUBBLOCK_BITS = BLOCK_BITS // 2

# Length of F-function subblock in bits.
F_SUBBLOCK_BITS = 8
...
# Modulo, in a group of which addition are held.
MOD = 2 ** 32

# Type alias for P array.
PArray = List[int]

# Hexadecimal digits of π (initializing value of P array)
PI_P_ARRAY = [
...
# Type alias for S-box.
SBox = List[int]

# Type alias for S-boxes.
SBoxes = List[SBox]

# Hexadecimal digits of π (initializing value of S-boxes)
PI_S_BOXES = [
    [
...
def _add(x1: int, x2: int) -> int:
    """Addition in the additive group (mod 2**32)."""
    return (x1 + x2) % MOD
...
def _process_subblocks(l: int, r: int, p: PArray, s: SBoxes,
                       is_encrypt: bool) -> Tuple[int, int]:
    """Process left and right subblocks using the P array and S-boxes with
    Blowfish cipher (works in both ways)."""
    if is_encrypt is True:
        rounds = range(ROUNDS_N)
        last_p_l, last_p_r = p[16:]
    else:
        rounds = range(ROUNDS_N + 1, 1, -1)
        last_p_l, last_p_r = p[1::-1]
...
def _ksa(key_ints: List[int]) -> Tuple[PArray, SBoxes]:
    """Generate the P array and S-boxes from the key
    ...
    for i in range(len(p)):
        p[i] ^= key_ints[i % key_len]

    l = r = 0
    for i in range(0, len(p), 2):
        p[i], p[i + 1] = _process_subblocks(l, r, p, s, True)

    for i in range(len(s)):
        for j in range(0, len(s[i]), 2):
            s[i][j], s[i][j + 1] = _process_subblocks(l, r, p, s, True)

    return p, s


class Blowfish(BlockCipher):
    """Class that encapsulates the Blowfish cipher's logic."""
    ...
    def _set_key(self, key: str, is_encrypt: bool) -> None:
        ...
        self.__p, self.__s = _ksa(key_ints)
